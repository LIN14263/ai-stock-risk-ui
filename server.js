...
async function finnhubHistory(ticker, range) {
  ...
  return { ticker, provider: "finnhub", range, points: series.length, series, ts: nowISO() };
}
...
async function yahooChart(ticker, range) {
  ...
  for (let i = 0; i < closes.length; i++) {
    ...
    if (typeof c === "number" && typeof t === "number") {
      series.push({ t: new Date(t * 1000).toISOString(), close: c });
    }
  }
  if (series.length < 2) throw new Error("Yahoo series too short");

  return { ticker, provider: "yahoo", range, points: series.length, series, ts: nowISO() };
}

// ---- Unified history only (your UI needs /api/history) ----
async function getHistoryUnified(ticker, range) {
  ...
  if (FINNHUB_API_KEY) {
    try { data = await finnhubHistory(ticker, r); } catch (e) { lastErr = e; }
  }
  if (!data) {
    try { data = await yahooChart(ticker, r); } catch (e) { lastErr = e; }
  }
...
app.get("/api/health", (req, res) => {
  okJson(res, { ok: true, ts: nowISO(), finnhubConfigured: Boolean(FINNHUB_API_KEY) });
});

app.get("/api/history", async (req, res) => {
  const ticker = cleanTicker(req.query.t);
  const range = String(req.query.range || "1M").toUpperCase();
  if (!ticker) return errJson(res, 400, "Missing ticker. Use /api/history?t=AAPL&range=1M");

  try {
    const h = await getHistoryUnified(ticker, range);
    okJson(res, h);
  } catch (e) {
    errJson(res, 502, "Failed to fetch history", { detail: String(e?.message || e) });
  }
});
...
app.listen(PORT, () => {
  console.log(`AI Stock Risk UI running: http://localhost:${PORT}`);
  console.log(`Health: http://localhost:${PORT}/api/health`);
  console.log(`Hist:   http://localhost:${PORT}/api/history?t=AAPL&range=1M`);
  if (!FINNHUB_API_KEY) console.log("NOTE: FINNHUB_API_KEY not set. Using Yahoo fallback (best-effort).");
});